 ...
 * @LastEditors: sueRimn
 * @LastEditTime: 2020-06-08 23:54:36
 */ 
#ifndef _libgenbank
#define _libgenbank
...
void get_CDS(char name[],FILE *target)          //进行CDS预处理
{
    ...
    while(fgets(line,MAXCOL,fp))               //读取文件直至CDS那一行
    {
        ...
        else if(strcmp(subline.str[0],"ORIGIN") == 0){
            free(subline.str);
            break;
        }
...
void output(FILE *fp,FILE *target)
{
    ...
    if (j < num_complement){
        for (; j < num_complement; ++j) get_sequence_p(section_start_c[j], section_end_c[j], fp, &line_num_, target);
    }
