 ...
 * @LastEditors: sueRimn
 * @LastEditTime: 2020-06-06 11:46:56
 */ 
#ifndef _libgenbank
#define _libgenbank
...
char defination[MAXCOL];
char note_fasta[MAXCOL];

int split(char *src, char *delim, IString* istr);     //分割函数
void Reverse(char s[]);                               //倒置函数
void tackle_join(FILE *fp,FILE *target);
void tackle_complement(FILE *fp,FILE *target);
void get_sequence(int begin,int end,FILE *fp,int* i,FILE *target);
void get_sequence_p(int begin,int end,FILE *fp,int *i,FILE *target);
void reverse_basic(FILE *fp,char basic);
void get_CDS(char name[],FILE *target);
void get_note(FILE *fp,FILE *target);           //读取的文件
...
void get_sequence_p(int begin,int end,FILE *fp,int *i,FILE *target) //处理complement序列读取
{
    ...
    do  //序列读取
    {
        if ((*i) == line_begin){
            ...
            for (;col_start < 10;++col_start)
            {
                reverse_basic(target,subline.str[subcol][col_start]);
            }
            subcol++;
            for (;subcol < 7;++subcol)
            {
                for (z = 0;z < 10;++z)
                    reverse_basic(target,subline.str[subcol][z]);
            }
            ...
            {
                for (z = 0;z < 10;++z)
                    reverse_basic(target,subline.str[col][z]);
            }
            int k = 0;
            for (;k <= col_over;++k)
            {
                reverse_basic(target,subline.str[subcol][k]);
            }
            free(subline.str);
            break;
        }
...
void reverse_basic(FILE *fp,char basic)
{
    if (basic == 'a') writeFasta_seq('t',fp);
    else if (basic == 't') writeFasta_seq('a',fp);
    else if (basic == 'c') writeFasta_seq('g',fp);
    else if (basic == 'g') writeFasta_seq('c',fp);
}
